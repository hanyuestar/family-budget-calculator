...
├── 开发改动记录.md / 测试报告.md / Git部署说明.md   # 项目文档
...
```bash
npm test            # 聚合运行 test_tarot.js + test_all.js，任一失败则整体失败
```

- `test_tarot.js`：**55** 条断言 —— 塔罗数据层（22 牌 / 3 阵 / 抽牌）+ 页面层（选型/抽牌/分享/历史）+ 集成出图层（卡面兜底/逆位旋转/长图不重叠）+ 保存链路回归。
- `test_all.js`：**128** 条断言 —— 6 个非塔罗工具的「核心计算逻辑 + 计算后按钮」全量回归（含财富健康指数的城市社保口径、个税累进、常识性校验等）。
- 合计 **183** 条断言；随功能增加断言数会同步增长。
...
> 文档维护：本 README 为项目对外指引的统一入口；更细的开发改动与测试记录见仓库内 `开发改动记录.md` 与 `测试报告.md`。
