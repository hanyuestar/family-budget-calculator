...
> 仅记录相对重要的版本变化；日常修复以 commit 历史为准。当前版本：`1.3.0`。

### 2026-08-17 · 内部质量优化（无用户可见变化）
依据 `代码审查与优化建议_20260817.md` 完成 7 项非破坏性优化，全量回归 **233/233** 通过（详见该报告）：
- **relation.js**：亲戚关系 `LOOKUP` 表去重，移除 54 条重复条目（保留唯一值与 4 个独有条目），逻辑零风险。
- **report.js**：抽离共用 `drawRoundRect`，消除品牌卡 / 码框 / `drawBadge` 三处重复圆角矩形样板。
- **calc-page.js**：`saveImage` 抽 `failSave` 统一失败兜底，降低回调嵌套与重复样板。
- **app.js**：`showInterstitial` 注册 `onClose` 前先 `offClose` 去重。
- **wealth-health.js**：抽 `defaultWealthHealthData()`，`data` 与 `reset` 共用默认对象（顺带补全 `cityNames` / `deductItems` / `isSaving` 复位）。
- **history.js**：`star()` 改用 `findIndex`。
- **format.js**：`formatMoney` 改用 `Intl.NumberFormat`，规避超大金额的科学计数法。

### 2026-07-29 · 财富健康指数取代「存钱段位」
- 下线「存钱段位」，新增「财富健康指数」（首页第 3 位）；接入 7 城社保 / 医保口径与 9 项专项附加扣除。

### 2026-07-17 · 国潮塔罗分包上线
- 新增「国潮塔罗」独立分包（22 张大阿尔卡纳 + 3 牌阵），置首页 1 号位；卡面图统一转 400×400 jpg 置于分包，根治部分 Android 真机无法解码 webp 的问题。
